...
public class TeamSpeakBridge
{
  ...
  {
    TeamSpeakUser user = TeamSpeakController.getInstance().getUser(clientName);
  ...
  public static void pokeClient(TeamSpeakUser user, String message)
  {
    ...
    String m = Color.cl("7") + "You poked " + Color.cl("9") + user.getNickName() + Color.cl("7") + ".";
    if (!message.isEmpty()) {
      m = Color.cl("7") + "You poked " + Color.cl("9") + user.getNickName() + Color.cl("7") + " with message: " + message;
    }
    TeamSpeak.addChat(null, null, m, EnumTargetMode.SERVER);
    TeamSpeak.addChat(null, null, m, EnumTargetMode.CHANNEL);
    if (TeamSpeak.selectedChat >= 0)
    {
      TeamSpeakUser u = TeamSpeakController.getInstance().getUser(TeamSpeak.selectedChat);
      if ((u != null) && 
        (u.getClientId() == user.getClientId())) {
        TeamSpeak.addChat(u, null, m, EnumTargetMode.USER);
      }
    }
  }
  ...
  {
    List<TeamSpeakUser> users = new ArrayList();
    users.addAll(TeamSpeakUser.getUsers());
    ...
    List<TeamSpeakUser> result = new ArrayList();
    for (TeamSpeakUser ts : users) {
      if (ts != null) {
        if (channelId == ts.getChannelId()) {
          result.add(ts);
        }
      }
    }
  ...
  public static void sendTextMessage(int id, String msg)
  {
    ...
    {
      TeamSpeakUser target = TeamSpeakController.getInstance().getUser(id);
      if (target != null) {
        messagePlayer(target, msg);
      } else {
